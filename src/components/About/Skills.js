...
const Skills = () => {
  return (
    <section className='w-full flex flex-col p-5 xs:p-10 sm:p-12 md:p-16 lg:p-20 border-b-2 border-solid border-dark dark:border-light text-dark'>
        <span className='font-semibold text-lg md:text-3xl lg:text-4xl text-accent dark:text-accentDark'>
            I'm comfortable in...
            <ul className='flex flex-wrap mt-8 justify-center xs:justify-start'>
            {
                SkillList.map((item, index) => {
                    return <li key={index} className='font-semibold dark:font-normal inline-block capitalize 
